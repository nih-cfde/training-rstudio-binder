...
For today's lesson, we will focus on data from the Gene Expression Tissue Project (GTEx). **To explore the data, navigate to the GTEX folder then click `GTEx.Rproj`. This will open up an Rproject and set the working directory to `~/GTEx/`.**
...
After installing packages, you must load them with the function `library()` to use the associated functions, objects, and help files.  In addition to using the package `ggplot`, we will also use `cowplot`,  `scales`, and `magick` to make pretty visualizations.
