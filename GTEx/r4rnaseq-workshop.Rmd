...
By the end of today's workshop, you create tables and plots like the ones below. These plots give a nice overview of the samples collected and the variables that can be used for differential gene expression analysis. 
...
3. Click `GTEx.Rproj` to  open up an Rproject which will set the working directory to `~/GTEx/`.
4. Open the **r4rnaseq-workshop.R** file which contains all the commands for today's workshop. 
...
_Note: the source document `r4rnaseq-workshop.Rmd` was last modified `r format(Sys.time(), '%d %B, %Y')`._
---
