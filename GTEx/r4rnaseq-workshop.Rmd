...
After installing packages, we need to load the functions and tools we want to use from the package with the `library()` command. In addition to using the package `ggplot`, we will also use `cowplot` and  `scales` to make pretty visualizations.
